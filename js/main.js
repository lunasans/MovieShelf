(function () {
    // Verhindere mehrfache Initialisierung (SPA-Redeclaration-Schutz)
    if (window.DVDApp) {
        if (window.IS_DEV) console.log('⚠️ DVDApp bereits definiert. Ausführung übersprungen.');
        return;
    }
    ...
    class DVDApp {
        ...
        handleDocumentClick(e) {
            ...
            // YouTube Trailer (aus trailers.php)
            const playBtn = e.target.closest('[data-action="play-trailer"]');
            if (playBtn) {
                e.preventDefault();
                this.openTrailerGallery(playBtn);
                return;
            }

            // Modal Close (Gallery)
            if (e.target.closest('.trailer-modal-close, .trailer-modal-backdrop')) {
                e.preventDefault();
                this.closeTrailerGallery();
                return;
            }

            // Play Video in Gallery
            if (e.target.closest('.modal-play-button')) {
                e.preventDefault();
                this.playGalleryVideo();
                return;
            }

            // YouTube Trailer (alt - aus film-view.php)
        ...
        handleKeydown(e) {
            ...
            // Barrierefreiheit für toggle-detail (Enter/Leertaste)
        ...
        // 🎬 TRAILER GALLERY LOGIC
        openTrailerGallery(element) {
            ...
            this.currentGalleryEmbed = embedUrl;
            ...
            if (coverImage && coverUrl) coverImage.src = coverUrl;
        ...
        playGalleryVideo() {
            if (!this.currentGalleryEmbed) return;
            const placeholder = document.getElementById('trailerPlaceholder');
            const videoContainer = document.getElementById('trailerVideoContainer');

            if (videoContainer) {
                videoContainer.innerHTML = `
                <iframe src="${this.currentGalleryEmbed}?autoplay=1&rel=0&modestbranding=1" 
        ...
        async loadPage(page) {
            ...
            try {
                ...
                // Re-initialize page-specific scripts if they exist
                if (page === 'actors' && typeof window.initActorsInfiniteScroll === 'function') {
                    window.initActorsInfiniteScroll();
                }

                if (page === 'trailers' && typeof window.initTrailersInfiniteScroll === 'function') {
                    window.initTrailersInfiniteScroll();
                }
        ...
        executeInlineScripts(c) {
            ...
            c.querySelectorAll('script').forEach(s => {
                // Core-Scripts überspringen, falls sie versehentlich in Partials auftauchen
                if (s.src && (s.src.includes('main.js') || s.src.includes('fancybox'))) {
                    if (window.IS_DEV) console.log('🚫 Core-Script in Partial übersprungen:', s.src);
                    return;
                }
                ...
                // Wichtig für SPA: Erzwungene Reihenfolge (synchronous execution behavior)
                n.async = false;
        ...
        initBoxSetModalDrag() {
            ...
            if (!c || m.dataset.dragInit) return;
            m.dataset.dragInit = "true";
            ...
            // Listeners auf window für flüssiges Ziehen auch außerhalb des Modals
    ...
    // Global Helpers registrieren
    window.DVDApp = DVDApp;

    // Initialisierung
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            if (!window.dvdApp) window.dvdApp = new DVDApp();
        });
    } else {
        if (!window.dvdApp) window.dvdApp = new DVDApp();
    }

    // Exponierte globale Funktionen (Kompatibilität)
    window.closeDetail = function () { if (window.dvdApp) window.dvdApp.closeDetail(); };
    ...
    window.closeBoxSetModal = function () { if (window.dvdApp) window.dvdApp.closeBoxSetModal(); };

    if (typeof window.IS_DEV === 'undefined') window.IS_DEV = false;
