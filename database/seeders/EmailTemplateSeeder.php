...
**Übrigens:** Für Fragen, Feature-Wünsche und Neuigkeiten gibt es unser [Community-Forum](https://forum.movieshelf.info) – du meldest dich dort einfach mit denselben Zugangsdaten an, ein separates Konto brauchst du nicht.
...
Danke,
{{ config(\'app.name\') }}',
            'variables_hint' => '$user->name, $tenantName, $deletionUrl'
        ]);
...
{{ config(\'app.name\') }}
        ...
        // firstOrCreate: im Cadmin angepasste Fassung wird bei erneutem Seeden nicht überschrieben
        EmailTemplate::firstOrCreate(['slug' => 'series_new_episodes'], [
            'name' => 'Serien: Neue Episoden',
            'subject' => 'Neue Episoden: {{ $seriesTitle }}',
            'content' => '<x-mail::message>
# Neue Episoden verfügbar

Hallo {{ $user->name }},

zu der Serie **{{ $seriesTitle }}** aus deiner Sammlung sind {{ $episodeCount }} neue Episode(n) erschienen:
...
Du möchtest keine E-Mails mehr zu neuen Episoden erhalten? [Hier abmelden]({{ $unsubscribeUrl }})
            ...
            'variables_hint' => '$user->name, $seriesTitle, $episodeCount, $episodeList, $seriesUrl, $unsubscribeUrl'
        ]);
