...
<div class="{{ $layoutMode === 'streaming' ? 'streaming-movie-details animate-in fade-in slide-in-from-bottom-8 duration-700' : 'animate-in fade-in slide-in-from-right-4 duration-500' }}"
    ...
    @if($layoutMode !== 'streaming')
        ...
        <div class="h-10 flex items-center mb-8 px-2">
            <h2 class="text-2xl font-black text-white flex items-center gap-4 tracking-tighter uppercase">
                <div class="h-10 w-2 bg-rose-500 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.5)]"></div>
                {{ $movie->title }}
            </h2>
        </div>
    @endif
            ...
            <div class="flex flex-col md:flex-row items-center md:items-end gap-12">
                ...
                <div class="flex-1 text-center md:text-left">
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-6">
                        <span class="px-6 py-2 bg-rose-600 rounded-2xl text-[10px] font-black tracking-[0.3em] uppercase shadow-xl shadow-rose-500/20">
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-12 text-white/40 text-xs font-bold uppercase tracking-[0.2em] italic">
                        @if($movie->runtime)
                            <div class="flex items-center gap-3">
                                <i class="bi bi-clock text-rose-500 text-lg"></i>
                    ...
                    @php
                        $conditionLabels = ['new' => 'Neu', 'like_new' => 'Wie neu', 'good' => 'Gut', 'acceptable' => 'Akzeptabel', 'damaged' => 'Beschädigt'];
                        $hasPhysical = $movie->edition || $movie->region_code || $movie->disc_location || $movie->purchase_date || $movie->purchase_price !== null || $movie->condition;
                    @endphp
                    @if($hasPhysical)
                        <div class="mt-8 flex flex-wrap items-center justify-center md:justify-start gap-2.5">
                            @if($movie->edition)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-award text-rose-400"></i>{{ $movie->edition }}</span>
                            @endif
                            @if($movie->region_code)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-globe text-rose-400"></i>Region {{ $movie->region_code }}</span>
                            @endif
                            @if($movie->disc_location)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-geo-alt-fill text-rose-400"></i>{{ $movie->disc_location }}</span>
                            @endif
                            @if($movie->condition)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-patch-check-fill text-rose-400"></i>{{ $conditionLabels[$movie->condition] ?? $movie->condition }}</span>
                            @endif
                            @if($movie->purchase_date)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-calendar-check text-rose-400"></i>{{ $movie->purchase_date->format('d.m.Y') }}</span>
                            @endif
                            @if($movie->purchase_price !== null)
                                <span class="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/70"><i class="bi bi-tag-fill text-rose-400"></i>{{ number_format((float) $movie->purchase_price, 2, ',', '.') }} €</span>
                            @endif
                        </div>
                    @endif
                </div>
    ...
    @if(isset($similarMovies) && $similarMovies->count() > 0)
        <div class="mb-8 pl-2">
            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <i class="bi bi-collection-play text-rose-400"></i>
                {{ __('Similar movies') }}
            </h3>
            <div class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory hide-scrollbar">
                @foreach($similarMovies as $similar)
                    <div @click="fetchDetails({{ $similar->id }}, '{{ $similar->backdrop_url }}')" class="w-24 shrink-0 snap-start cursor-pointer group/similar">
                        <div class="w-full aspect-[2/3] bg-gray-900 rounded-xl overflow-hidden glass border border-white/5 group-hover/similar:border-rose-500/30 transition-all shadow-lg mb-2 relative">
                        ...
                        <div class="text-[10px] font-bold text-gray-400 truncate group-hover/similar:text-rose-400 transition-colors">
                            {{ $similar->title }}
                        </div>
    ...
    <div class="space-y-8">
        ...
        @if($movie->overview)
            <div class="glass p-6 md:p-12 rounded-[3rem] border-white/5 mb-12">
                ...
                <div class="relative">
                    <div class="absolute -left-8 top-0 bottom-0 w-1 bg-rose-600 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.5)] opacity-50"></div>
            ...
            <div class="flex items-center justify-between mb-8 px-2">
                ...
                @if($movie->actors->count() > 4)
                    <div class="flex items-center gap-2">
                        <button @click="scroll(-1)" class="w-12 h-12 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-rose-500/50 flex items-center justify-center transition-all active:scale-90 text-gray-500 hover:text-white">
            ...
            <div class="flex gap-6 overflow-x-auto pb-8 no-scrollbar scroll-smooth" x-ref="castContainer">
                @forelse($movie->actors as $actor)
                    <div @click="fetchActor({{ $actor->id }})" 
                        ...
                        <div class="relative aspect-square rounded-[2rem] overflow-hidden glass-streaming border border-white/5 transition-all duration-700 hover:scale-[1.08] hover:border-rose-500/50 shadow-2xl mb-4">
                        ...
                        <div class="px-2">
                             <div class="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1 italic">{{ $actor->pivot->role ?: __('Actors') }}</div>
        ...
        @if($movie->boxsetChildren->count() > 0)
            <div class="glass p-6 rounded-3xl border-white/5">
                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2 underline decoration-rose-500/50 underline-offset-8">
                    <i class="bi bi-collection-play text-rose-400"></i>
                    {{ __('Movies in this set') }} ({{ $movie->boxsetChildren->count() }})                </h3>
                <div class="space-y-3">
                    @foreach($movie->boxsetChildren as $child)
                        <div @click="fetchDetails({{ $child->id }})" class="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group/child">
                            <div class="w-8 h-12 bg-gray-800 rounded-lg flex items-center justify-center flex-shrink-0 border border-white/5 group-hover/child:border-rose-500/30 overflow-hidden">
                                @if($child->cover_url)
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="text-xs font-bold text-white truncate group-hover/child:text-rose-400 transition-colors">{{ $child->title }}</div>
                                <div class="text-[10px] text-gray-500 uppercase">{{ $child->year }} • {{ $child->collection_type }}</div>
                            </div>
                            <i class="bi bi-chevron-right text-gray-700 text-xs mr-2 group-hover/child:text-rose-400 transition-colors"></i>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        @if($movie->collection_type === 'Serie' && $movie->seasons->count() > 0)
            ...
            <div class="glass p-6 rounded-3xl border-white/5"
                ...
                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <i class="bi bi-layers accent-text"></i>
                    {{ __('Seasons & episodes') }}
                </h3>
                <div class="space-y-4">
                    @foreach($movie->seasons->sortBy('season_number') as $season)
                        ...
                        <div class="rounded-2xl border border-white/5 bg-white/5 overflow-hidden transition-colors duration-300"
                             :class="activeSeason === {{ $season->id }} ? 'accent-soft-border bg-white/10' : 'hover:border-white/10'">
                            <div class="w-full flex items-center justify-between p-4 text-left group gap-3">
                                <button @click="activeSeason = activeSeason === {{ $season->id }} ? null : {{ $season->id }}"
                                        class="flex items-center gap-4 flex-1 min-w-0">
                                    <div class="w-10 h-10 accent-soft-bg accent-text rounded-xl flex items-center justify-center font-black shrink-0">
                                        {{ $season->season_number }}
                                    </div>
                                    <div class="min-w-0">
                                        <div class="text-sm font-black text-white transition-colors truncate">{{ $season->title ?: __('Season ' . $season->season_number) }}</div>
                                        <div class="flex items-center gap-2 mt-1">
                                            ...
                                            <span class="text-[10px] text-gray-500 uppercase font-bold tracking-widest whitespace-nowrap">
                                ...
                                <button @click="toggleSeason({{ $season->id }}, @js($seasonEpIds))" type="button"
                                        class="shrink-0 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border transition-all"
                                        :class="seasonWatched(@js($seasonEpIds)) === {{ $season->episodes->count() }} ? 'accent-soft-bg accent-soft-border accent-text' : 'border-white/10 text-gray-400 hover:text-white hover:border-white/30'"
                                ...
                                <button @click="activeSeason = activeSeason === {{ $season->id }} ? null : {{ $season->id }}" class="shrink-0">
                                    <i class="bi text-gray-500 transition-transform duration-300"
                                       :class="activeSeason === {{ $season->id }} ? 'bi-chevron-up rotate-180 accent-text' : 'bi-chevron-down'"></i>
                                </button>
                            </div>
                            <div x-show="activeSeason === {{ $season->id }}"
                                ...
                                <div class="divide-y divide-white/5">
                                    @foreach($season->episodes->sortBy('episode_number') as $episode)
                                        <div class="p-4 hover:bg-white/5 transition-colors" :class="isWatched({{ $episode->id }}) ? 'opacity-60' : ''">
                                            <div class="flex items-center gap-3 mb-1">
                                                <button @click="toggleEp({{ $episode->id }})" type="button"
                                                        class="shrink-0 w-5 h-5 rounded-full border flex items-center justify-center transition-all"
                                                ...
                                                <span class="text-[10px] font-black accent-text opacity-50 w-6 shrink-0">E{{ $episode->episode_number }}</span>
                                                <h4 class="text-xs font-bold text-white">{{ $episode->title }}</h4>
                                                <span x-show="isNext({{ $episode->id }})" x-cloak
                                                      class="text-[8px] font-black uppercase tracking-widest accent-soft-bg accent-text px-2 py-0.5 rounded-full">{{ __('Next') }}</span>
                                                @if($episode->runtime)
                                                    <span class="text-[9px] text-gray-600 font-bold ml-auto shrink-0">{{ $episode->runtime }} Min.</span>
                                                @endif
                                            </div>
                                            @if($episode->overview)
                                                <p class="text-[10px] text-gray-500 leading-relaxed ml-8 line-clamp-2 prose-movie">
    ...
    <div class="mt-8">
        ...
        <div class="flex items-center justify-between mt-6 px-1">
            <div class="flex items-center gap-3">
                <span class="text-[10px] font-black text-white/30 uppercase tracking-widest">{{ __('Your rating') }}</span>
            ...
            <div class="flex items-center gap-2 text-[10px] text-white/20 font-bold" x-show="ratingCount > 0">
                ...
                <span x-text="ratingCount + ' ' + '{{ __('Ratings') }}' + ' · Ø ' + avgRating"></span>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 mt-4">
            ...
            <button @click="toggleWishlist()"
                ...
                <span class="text-xs font-bold uppercase tracking-widest transition-colors"
                    ...
                    <span x-show="isWishlisted">{{ __('On watchlist') }}</span>
                    <span x-show="!isWishlisted">{{ __('Remember') }}</span>
                </span>
            ...
            <button @click="toggleWatched()"
                    class="h-14 px-6 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl flex items-center gap-3 transition-all group shadow-lg"
                    :class="isWatched ? 'border-rose-500/50 bg-rose-500/10' : ''">
                <span class="text-xs font-bold uppercase tracking-widest transition-colors"
                      :class="isWatched ? 'text-rose-400' : 'text-gray-400 group-hover:text-white'">
                    <span x-show="isWatched">{{ __('Watched') }}</span>
                    <span x-show="!isWatched">{{ __('Mark as watched') }}</span>
                </span>
                <i class="bi text-xl transition-colors"
                   :class="isWatched ? 'bi-eye-fill text-rose-400' : 'bi-eye text-gray-400 group-hover:text-rose-400'"></i>
            </button>
        </div>
    </div>
</div>