<div class="streaming-container animate-in fade-in duration-700 overflow-x-hidden">
    ...
    <section class="relative h-[85vh] w-full rounded-b-[3rem] mb-0 group"
        ...
        <div class="absolute bottom-12 right-12 z-20 flex gap-3">
            ...
            <button @click="active = {{ $index }}" 
                    ...
                    :class="active === {{ $index }} ? 'w-12 bg-rose-600' : 'w-4 bg-white/20 hover:bg-white/40'"></button>
            @endforeach
        </div>
        @endif
                ...
                <div class="relative group">
                    <form action="{{ route('dashboard') }}" method="GET" class="relative transition-all duration-700 ease-in-out transform">
                        <input type="text" name="q" value="{{ request('q') }}"
                            ...
                            class="w-full bg-white/10 border-y border-white/20 py-4 px-12 pl-20 focus:ring-0 focus:border-rose-500/50 text-xl md:text-2xl transition-all placeholder:text-gray-400 backdrop-blur-3xl group-hover:bg-white/15 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.7)] text-white font-light tracking-wide outline-none"
                            :class="isSearchFocused ? 'bg-white/20 border-rose-500/50 shadow-[0_0_80px_rgba(59,130,246,0.3)]' : ''"
                        >
                        <div class="absolute left-10 top-1/2 -translate-y-1/2 flex items-center justify-center transition-all duration-500" :class="isSearchFocused ? 'scale-110 text-rose-400' : 'text-gray-500'">
                            <i class="bi bi-search text-2xl group-hover:text-rose-400 transition-colors"></i>
                        </div>
    ...
    {{-- Content Rows --}}
    <div class="space-y-16 pb-4" style="padding-top: 2.5rem;">
        ...
        <div class="px-12 md:px-20 mt-6">
            <form action="{{ route('dashboard') }}" method="GET"
                  class="glass border {{ $hasActiveFilters ? 'border-rose-500/30' : 'border-white/10' }} rounded-2xl p-5 w-full"
                ...
                <div class="flex items-center gap-2 text-gray-500 self-center shrink-0 pr-2 border-r border-white/10">
                    <i class="bi bi-funnel-fill text-sm {{ $hasActiveFilters ? 'text-rose-400' : '' }}"></i>
                    <span class="text-[9px] font-black uppercase tracking-widest {{ $hasActiveFilters ? 'text-rose-400' : '' }}">Filter</span>
                </div>

                <div style="display:flex;flex-direction:column;gap:0.375rem;">
                    <label class="text-[9px] font-black text-white/30 uppercase tracking-widest px-1">Genre</label>
                    <select name="genre" class="w-full bg-white/5 border {{ request('genre') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 appearance-none cursor-pointer">
                ...
                <div style="display:flex;flex-direction:column;gap:0.375rem;">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="year_from" value="{{ request('year_from') }}" placeholder="von" min="1900" max="{{ date('Y') }}"
                            class="w-full bg-white/5 border {{ request('year_from') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 placeholder:text-gray-600">
                        ...
                        <input type="number" name="year_to" value="{{ request('year_to') }}" placeholder="bis" min="1900" max="{{ date('Y') }}"
                            class="w-full bg-white/5 border {{ request('year_to') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 placeholder:text-gray-600">
                    </div>
                </div>
                ...
                <div style="display:flex;flex-direction:column;gap:0.375rem;">
                    ...
                    <input type="number" name="rating_min" value="{{ request('rating_min') }}" placeholder="0–10" min="0" max="10" step="0.5"
                        class="w-full bg-white/5 border {{ request('rating_min') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 placeholder:text-gray-600">
                </div>
                ...
                <div style="display:flex;flex-direction:column;gap:0.375rem;">
                    ...
                    <input type="number" name="runtime_max" value="{{ request('runtime_max') }}" placeholder="z.B. 120" min="1"
                        class="w-full bg-white/5 border {{ request('runtime_max') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 placeholder:text-gray-600">
                </div>

                <div class="flex gap-2 self-end">
                    <button type="submit" class="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all">
                        <i class="bi bi-funnel"></i>
                    </button>
                    @if($hasActiveFilters)
                        <a href="{{ route('dashboard', array_filter(['q' => request('q'), 'type' => request('type'), 'media' => request('media')])) }}"
        ...
        @if(request('q') || request('type') || request('media') === 'tv' || $hasActiveFilters)
        ...
        <section x-data="{ 
            ...
            <button @click="$refs.slider.scrollBy({ left: -600, behavior: 'smooth' })" 
                    ...
                    class="absolute left-4 top-[calc(50%+1rem)] -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-rose-600 shadow-2xl">
            ...
            <button @click="$refs.slider.scrollBy({ left: 600, behavior: 'smooth' })" 
                    ...
                    x-transition:enter-start="opacity-0 translate-x-4"
                    x-transition:enter-end="opacity-100 translate-x-0"
                    class="absolute right-4 top-[calc(50%+1rem)] -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-rose-600 shadow-2xl">
            ...
            <div x-ref="slider" 
                ...
                @foreach($latestMovies as $movie)
                     <div @click="window.location.href = '{{ route('movies.show', $movie) }}'" 
                         class="w-[160px] md:w-[220px] shrink-0 aspect-[2/3] relative rounded-[2rem] overflow-hidden glass-streaming border border-white/10 group cursor-pointer hover:border-rose-500/50 hover:scale-105 transition-all duration-500 shadow-2xl">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                             ...
                             <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                 <span class="text-[8px] font-black text-rose-400 uppercase tracking-widest">{{ $movie->year }}</span>
        ...
        {{-- Row: Weiterschauen (angefangene Serien, persönlich) --}}
        @if(($continueWatching ?? collect())->isNotEmpty())
        <section class="relative">
            <div class="flex items-center justify-between mb-6 px-12 md:px-20">
                <h2 class="text-2xl font-black text-white tracking-tight flex items-center gap-4">
                    <span>{{ __('Keep watching') }}</span>
                    <div class="h-1 w-12 accent-fill rounded-full"></div>
                </h2>
            </div>
            <div class="flex gap-6 overflow-x-auto no-scrollbar pb-8 px-12 md:px-20 scroll-smooth">
                @foreach($continueWatching as $serie)
                    <div @click="window.location.href = '{{ route('movies.show', $serie) }}'"
                         class="w-[300px] md:w-[360px] shrink-0 relative rounded-[2rem] overflow-hidden glass-streaming border border-white/10 group cursor-pointer hover:border-white/30 hover:scale-[1.02] transition-all duration-500 shadow-2xl">
                        <div class="aspect-video relative">
                            @if($serie->backdrop_url ?? null)
                                <img src="{{ $serie->backdrop_url }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                            @elseif($serie->cover_url)
                                <img src="{{ $serie->cover_url }}" class="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-700">
                            @else
                                <div class="w-full h-full flex items-center justify-center bg-white/5 text-gray-700"><i class="bi bi-tv text-4xl"></i></div>
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent"></div>
                            <div class="absolute bottom-0 left-0 right-0 p-5">
                                <h4 class="text-sm font-black text-white uppercase tracking-wider truncate drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{{ $serie->title }}</h4>
                                @if($serie->next_episode)
                                    <p class="text-[10px] font-bold text-white/60 mt-1">
                                        {{ __('Continue with') }}
                                        <span class="accent-text">S{{ $serie->next_episode->season_number }}E{{ $serie->next_episode->episode_number }}</span>
                                        @if($serie->next_episode->title)
                                            &middot; {{ \Illuminate\Support\Str::limit($serie->next_episode->title, 30) }}
                                        @endif
                                    </p>
                                @endif
                            </div>
                        ...
                        <div class="h-1.5 bg-white/10">
                            <div class="h-full accent-fill" style="width: {{ $serie->progress_percent }}%"></div>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        {{-- Row: Serien (getrennt von den Filmen) --}}
        @if(($series ?? collect())->isNotEmpty())
        <section x-data="{
            canScrollLeft: false,
            canScrollRight: true,
            checkScroll() {
                const el = this.$refs.seriesSlider;
                this.canScrollLeft = el.scrollLeft > 10;
                this.canScrollRight = el.scrollLeft < (el.scrollWidth - el.clientWidth - 10);
            }
        }" x-init="checkScroll()" class="relative group/row">
            <div class="flex items-center justify-between mb-6 px-12 md:px-20">
                ...
                <a href="{{ route('dashboard', ['media' => 'tv']) }}" class="text-sm font-bold text-gray-500 hover:text-white transition-colors">{{ __('View All') }}</a>
            </div>

            <button @click="$refs.seriesSlider.scrollBy({ left: -600, behavior: 'smooth' })"
                    x-show="canScrollLeft"
                    x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="opacity-0 -translate-x-4"
                    x-transition:enter-end="opacity-100 translate-x-0"
                    class="absolute left-4 top-[calc(50%+1rem)] -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-purple-600 shadow-2xl">
                <i class="bi bi-chevron-left text-xl"></i>
            </button>
            ...
            <button @click="$refs.seriesSlider.scrollBy({ left: 600, behavior: 'smooth' })"
                    ...
                    x-transition:enter-start="opacity-0 translate-x-4"
                    x-transition:enter-end="opacity-100 translate-x-0"
                    class="absolute right-4 top-[calc(50%+1rem)] -translate-y-1/2 z-30 w-12 h-12 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-purple-600 shadow-2xl">
                <i class="bi bi-chevron-right text-xl"></i>
            </button>

            <div x-ref="seriesSlider"
                 @scroll.debounce.50ms="checkScroll()"
                 class="flex gap-6 overflow-x-auto no-scrollbar pb-8 px-12 md:px-20 scroll-smooth">
                @foreach($series as $serie)
                     <div @click="window.location.href = '{{ route('movies.show', $serie) }}'"
                         class="w-[160px] md:w-[220px] shrink-0 aspect-[2/3] relative rounded-[2rem] overflow-hidden glass-streaming border border-white/10 group cursor-pointer hover:border-purple-500/50 hover:scale-105 transition-all duration-500 shadow-2xl">
                        <img src="{{ $serie->cover_url }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-60 transition-opacity duration-500 group-hover:opacity-0"></div>
                        <div class="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                        <div class="absolute bottom-0 left-0 right-0 p-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                             <h4 class="text-xs font-black text-white uppercase tracking-wider mb-2 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] truncate">{{ $serie->title }}</h4>
                             <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                 <span class="text-[8px] font-black text-purple-400 uppercase tracking-widest">{{ $serie->year }}</span>
                                 <div class="h-1 w-1 bg-white/20 rounded-full"></div>
                                 <span class="text-[8px] font-black text-white/40 uppercase tracking-widest">{{ __('Series') }}</span>
                             </div>
                        </div>
                    </div>
                @endforeach
        ...
        <section class="animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
            ...
            <div x-ref="movieGrid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-8 px-12 md:px-20 pb-8">
            ...
            <div x-show="nextMoviesPageUrl"
                 class="pb-4 flex flex-col items-center justify-center gap-6">
                <div x-show="isMoviesLoading" class="flex flex-col items-center gap-4">
                    <div class="w-12 h-12 border-4 border-rose-500/20 border-t-rose-500 rounded-full animate-spin"></div>
                ...
                <button x-show="!isMoviesLoading" @click="loadMoreMovies()" 
                        class="px-12 py-5 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl text-xs font-black text-white uppercase tracking-[0.3em] hover:bg-white/10 hover:border-rose-500/50 hover:scale-105 transition-all shadow-2xl group active:scale-95">
                    <span>{{ __('Load more movies') }}</span>
                    <i class="bi bi-chevron-down ml-3 group-hover:translate-y-1 transition-transform inline-block text-rose-500"></i>
                </button>
            </div>
        </section>
        @endif
