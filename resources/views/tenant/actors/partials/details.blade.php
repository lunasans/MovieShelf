    ...
    @if($layoutMode === 'streaming')
        ...
        <div class="relative mb-16 rounded-[3rem] overflow-hidden group shadow-2xl border border-white/10 min-h-[500px]">
             ...
             <div class="absolute inset-0 bg-[#0c0c0e]">
                ...
                @else
                    <div class="absolute inset-0 bg-gradient-to-br from-rose-600/10 to-purple-600/10 opacity-30"></div>
                @endif
             </div>
             ...
             <div class="relative z-10 p-12 lg:p-20 flex flex-col lg:flex-row gap-16 items-center lg:items-start text-center lg:text-left">
                ...
                <div class="flex-1 pt-4">
                    ...
                    <h2 class="text-6xl lg:text-8xl font-black text-white leading-[0.9] tracking-tighter mb-8 uppercase italic drop-shadow-2xl">
                        {{ $actor->first_name }}<br>
                        <span class="text-rose-500 selection:bg-white selection:text-rose-600">{{ $actor->last_name }}</span>
                    </h2>
                    @if($actor->original_name)
                        <p class="text-xl text-white/30 font-bold tracking-widest -mt-4 mb-8">{{ $actor->original_name }}</p>
                    @endif
                    ...
                    <div class="flex flex-wrap items-center justify-center lg:justify-start gap-12 text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-12 italic">
                        <div class="flex items-center gap-3">
                            <i class="bi bi-film text-rose-500 text-lg"></i>
                            <span>{{ $stats['total_movies'] }} {{ __('Movies') }}</span>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span>{{ number_format($actor->view_count) }} {{ __('Views') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center justify-center lg:justify-start gap-6">
                        ...
                        <a href="{{ route('actors.show', $actor) }}" target="_blank" 
                           class="w-14 h-14 bg-rose-600 rounded-[1.5rem] flex items-center justify-center text-white shadow-xl shadow-rose-500/20 hover:scale-110 active:scale-95 transition-all group">
        ...
        @if($actor->bio)
            <div class="mb-20 animate-in fade-in duration-1000 delay-300">
                <div class="flex items-center gap-8 mb-10">
                    <h3 class="text-xs font-black text-white/20 uppercase tracking-[0.5em] italic shrink-0">{{ __('Biography') }}</h3>
        ...
        <div class="mb-10 flex items-center justify-between">
             <h3 class="text-xs font-black text-white/20 uppercase tracking-[0.5em] italic">{{ __('Collection') }}</h3>
             <span class="text-[10px] font-black text-rose-500 uppercase tracking-widest italic animate-pulse group-hover:animate-none">
                 {{ count($movies) }} {{ __('productions found') }}
             </span>
        </div>
        ...
        <div class="flex gap-6 overflow-x-auto no-scrollbar pb-10">
            @foreach ($movies as $movie)
                <div @click="window.location.href = '{{ route('movies.show', $movie) }}'" 
                     class="w-[180px] md:w-[220px] shrink-0 group cursor-pointer animate-in zoom-in duration-700">
                    <div class="relative aspect-[2/3] rounded-[2rem] overflow-hidden glass-streaming border border-white/5 transition-all duration-700 hover:scale-[1.08] hover:border-rose-500/50 shadow-2xl">
                        @if ($movie->cover_url)
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                             <span class="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1 italic">{{ $movie->pivot->role ?? 'Darsteller' }}</span>
    ...
    @else
            ...
            <div class="absolute left-8 bottom-8 flex items-end gap-8 z-20">
                ...
                <div class="pb-2">
                    <div class="flex items-center gap-3 mb-3">
                        <span class="px-3 py-1 bg-rose-600/80 backdrop-blur-md rounded-lg text-[10px] font-black tracking-widest uppercase border border-white/20 shadow-lg">{{ __('Actor') }}</span>
                    </div>
                    <h2 class="text-3xl font-black text-white leading-tight mb-2 drop-shadow-2xl tracking-tighter uppercase">{{ $actor->full_name }}</h2>
                    @if($actor->original_name)
                        <p class="text-sm text-gray-400 font-bold tracking-widest mb-2">{{ $actor->original_name }}</p>
                    @endif
                    <div class="flex items-center gap-6 text-gray-300 text-xs font-medium">
                        <div class="flex items-center gap-2 italic">
                            <i class="bi bi-film text-rose-400"></i>
                            <span>{{ $stats['total_movies'] }} {{ __('Movies') }}</span>
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-3 gap-4 mb-8">
            <div class="glass p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                <span class="text-gray-500 text-[9px] font-bold uppercase tracking-widest mb-1">{{ __('Leading roles') }}</span>
                <div class="text-xl font-bold text-white">{{ $stats['main_roles'] }}</div>
            </div>
            <div class="glass p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                <span class="text-gray-500 text-[9px] font-bold uppercase tracking-widest mb-1">{{ __('Time span') }}</span>
                <div class="text-sm font-bold text-rose-400">{{ $stats['year_span'] ?? 'N/A' }}</div>
            </div>
            <div class="glass p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                <span class="text-gray-500 text-[9px] font-bold uppercase tracking-widest mb-1">{{ __('Views') }}</span>
