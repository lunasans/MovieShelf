...
<x-app-layout>
    ...
    <div class="{{ $layoutMode === 'streaming' ? 'streaming-actor-profile min-h-screen pt-32 pb-20 px-4 md:px-12 lg:px-24 relative overflow-hidden' : 'relative min-h-screen' }}">
                 ...
                 <div class="flex flex-col lg:flex-row gap-20 items-center lg:items-start">
                     ...
                     <div class="w-full lg:w-1/3 max-w-[450px]">
                         <div class="relative group">
                            <div class="absolute -inset-6 bg-rose-500/20 rounded-[3rem] blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
                         ...
                         <div class="grid grid-cols-2 gap-6 mt-12 animate-in slide-in-from-bottom duration-700 delay-300">
                             <div class="glass-streaming p-6 rounded-[2rem] border border-white/10 flex flex-col items-center justify-center text-center">
                                ...
                                <span class="text-[9px] font-black text-white/30 uppercase tracking-[0.3em] mt-2 italic">{{ __('Movies') }}</span>
                             </div>
                             <div class="glass-streaming p-6 rounded-[2rem] border border-white/10 flex flex-col items-center justify-center text-center group hover:bg-rose-500/10 transition-colors">
                                <span class="text-rose-500 text-3xl font-black italic tracking-tighter group-hover:scale-110 transition-transform">{{ $actor->view_count }}</span>
                                <span class="text-[9px] font-black text-white/30 uppercase tracking-[0.3em] mt-2 italic">{{ __('Views') }}</span>
                             </div>
                         </div>
                     </div>
                     ...
                     <div class="flex-1 w-full">
                         <div class="mb-16 animate-in slide-in-from-right duration-1000">
                            <h1 class="text-7xl md:text-9xl font-black text-white tracking-tighter leading-[0.8] mb-8 uppercase drop-shadow-2xl italic">
                                {{ $actor->first_name }}<br>
                                <span class="text-rose-500 drop-shadow-[0_0_50px_rgba(59,130,246,0.3)]">{{ $actor->last_name }}</span>
                            </h1>
                            @if($actor->original_name)
                                <p class="text-2xl text-white/30 font-bold tracking-widest mb-8">{{ $actor->original_name }}</p>
                            @endif
                            <div class="h-2 w-32 bg-rose-600 rounded-full shadow-[0_0_30px_rgba(59,130,246,0.7)] mb-10"></div>
                         ...
                         @if($actor->bio)
                            <div class="mb-20 animate-in fade-in duration-1000 delay-500">
                                ...
                                <div class="text-white/70 leading-relaxed text-xl lg:text-2xl font-medium tracking-tight relative pl-10 prose-movie">
                                    <span class="absolute left-0 top-0 text-7xl text-rose-500/20 leading-none">"</span>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
                                @foreach($movies as $movie)
                                    <a href="{{ route('dashboard', ['movie' => $movie->id]) }}" class="group flex flex-col gap-5 animate-in zoom-in duration-700">
                                        <div class="relative aspect-[2/3] rounded-[2.5rem] overflow-hidden glass-streaming border border-white/5 transition-all duration-700 hover:scale-[1.08] hover:border-rose-500/50 shadow-2xl">
                                            @if($movie->cover_url)
                                            ...
                                            <div class="absolute inset-0 flex flex-col justify-center items-center p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-100 bg-black/60 backdrop-blur-sm z-30">
                                                 <div class="w-12 h-12 bg-rose-600 rounded-full flex items-center justify-center text-white shadow-xl mb-4">
                                        ...
                                        <div class="px-2 text-center lg:text-left">
                                            <h4 class="text-white font-black text-sm leading-none truncate group-hover:text-rose-400 transition-colors uppercase tracking-tight italic">{{ $movie->title }}</h4>
        ...
        @else
            <!-- Legacy Classic Mode -->
            <div class="absolute inset-0 h-[60vh] bg-gradient-to-b from-rose-600/10 to-transparent opacity-30 pointer-events-none"></div>
                ...
                <div class="flex flex-col lg:flex-row gap-16">
                     <div class="w-full lg:w-1/3 flex flex-col items-center lg:items-start text-center lg:text-left">
                        ...
                        <div class="w-full glass p-8 rounded-3xl border border-white/10">
                             <h2 class="text-xs font-black uppercase tracking-[0.3em] text-rose-400 mb-4">{{ __('Details') }}</h2>
                             <span class="text-white font-bold">{{ number_format($actor->view_count) }} {{ __('Views') }}</span>
                        </div>
                     </div>
                     <div class="w-full lg:w-2/3">
                        <h1 class="text-6xl md:text-8xl font-black text-white tracking-tighter mb-4 uppercase">{{ $actor->full_name }}</h1>
                        @if($actor->original_name)
                            <p class="text-xl text-gray-500 font-bold tracking-widest mb-6">{{ $actor->original_name }}</p>
                        @endif
