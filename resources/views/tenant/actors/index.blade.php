...
<x-app-layout>
    ...
    <div class="{{ $layoutMode === 'streaming' ? 'streaming-actors-view min-h-screen pt-0 pb-20 px-12 md:px-20 relative' : 'px-8 py-10 min-h-screen' }}"
         x-data="actorGallery()">
            ...
            <div class="relative z-10">
                ...
                <div class="mb-16 animate-in slide-in-from-left duration-700">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                        ...
                        <form action="{{ route('actors.index') }}" method="GET" class="relative w-full max-w-md group">
                            ...
                            <div class="absolute -inset-1 bg-gradient-to-r from-rose-600 to-purple-600 rounded-2xl blur opacity-20 group-focus-within:opacity-50 transition duration-500"></div>
                            ...
                            <i class="bi bi-search absolute left-6 top-1/2 -translate-y-1/2 text-white/30 text-xl group-focus-within:text-rose-500 transition-colors"></i>
                        </form>
                    </div>
                </div>
                ...
                <div class="relative z-20 mb-20 animate-in fade-in duration-1000 delay-300">
                    <div class="glass-streaming p-3 rounded-[2rem] border border-white/10 flex items-center gap-2 overflow-x-auto alphabet-nav px-4">
                        <a href="{{ route('actors.index', ['q' => request('q')]) }}"
                           class="px-8 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all {{ !$letter ? 'bg-rose-600 text-white shadow-xl shadow-rose-500/30 grow-0 shrink-0' : 'text-white/40 hover:text-white hover:bg-white/5 grow-0 shrink-0' }}">
                            {{ __('All') }}
                        </a>
                        ...
                        @foreach(range('A', 'Z') as $char)
                            ...
                            <a href="{{ $hasActors ? route('actors.index', ['letter' => $char, 'q' => request('q')]) : '#' }}"
                               ...
                               {{ $letter === $char ? 'bg-rose-600 text-white shadow-xl shadow-rose-500/30' : ($hasActors ? 'text-white/40 hover:bg-white/10 hover:text-white' : 'text-white/10 cursor-not-allowed') }}">
                                {{ $char }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div x-show="nextPageUrl" class="mt-24 flex flex-col items-center justify-center gap-6">
                    <div x-show="isLoading" class="flex flex-col items-center gap-6 animate-in fade-in duration-500">
                        <div class="w-16 h-16 border-4 border-rose-500/20 border-t-rose-500 rounded-full animate-spin shadow-[0_0_30px_rgba(59,130,246,0.3)]"></div>
                        <span class="text-[12px] font-black text-white/30 uppercase tracking-[0.4em] italic animate-pulse">{{ __('Loading more actors...') }}</span>
                    </div>
                    <button x-show="!isLoading" @click="loadMore()" class="glass-streaming px-12 py-5 border border-white/10 rounded-3xl text-xs font-black text-white/40 hover:text-white hover:border-rose-500 transition-all uppercase tracking-[0.3em] italic group shadow-2xl">
            ...
            <div class="max-w-[90rem] mx-auto">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 uppercase">
                    <div>
                        <h1 class="text-4xl font-black text-white tracking-tighter mb-2 italic">
                            {!! __('Our <span class="text-rose-500">Stars</span>') !!}
                        </h1>
                        <div class="flex items-center gap-3">
                            <span class="px-3 py-1 bg-rose-500/10 border border-rose-500/20 rounded-full text-[10px] font-black text-rose-400 uppercase tracking-widest italic">
                ...
                <div class="glass p-4 rounded-2xl border border-white/10 mb-12 overflow-x-auto no-scrollbar">
                    <div class="flex items-center justify-between gap-2 min-w-max px-2">
                        <a href="{{ route('actors.index', ['q' => request('q')]) }}" class="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ !$letter ? 'bg-rose-600 text-white' : 'text-gray-500 hover:text-white' }}">{{ __('All') }}</a>
                        @foreach(range('A', 'Z') as $char)
                            @php $hasActors = in_array($char, $availableLetters); @endphp
                            <a href="{{ $hasActors ? route('actors.index', ['letter' => $char, 'q' => request('q')]) : '#' }}" class="w-10 h-10 flex items-center justify-center rounded-xl text-xs font-black transition-all {{ $letter === $char ? 'bg-rose-600 text-white' : ($hasActors ? 'text-gray-300 hover:bg-white/10' : 'text-gray-700 cursor-not-allowed') }}">{{ $char }}</a>
                        @endforeach
                    </div>
                </div>
