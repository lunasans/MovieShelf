 <x-app-layout>
    ...
    >
        ...
        <section class="film-list-area shadow-2xl">
                @include('tenant.partials.onboarding-checklist')
                <div class="flex flex-col gap-6 mb-4">
                    ...
                    <div class="relative group max-w-lg px-2">
                        <form action="{{ route('dashboard') }}" method="GET" class="relative transition-all duration-500 ease-in-out">
                            <input type="text" name="q" value="{{ request('q') }}"
                                ...
                                class="w-full bg-white/10 border border-white/20 rounded-[2rem] py-2 px-6 pl-14 focus:ring-8 focus:ring-rose-500/20 focus:border-rose-500/50 text-base transition-all placeholder:text-gray-400 backdrop-blur-2xl group-hover:bg-white/15 shadow-2xl text-white"
                                :class="isSearchFocused ? 'bg-white/20 border-rose-500/50 shadow-[0_0_40px_rgba(59,130,246,0.3)]' : ''"
                            >
                            <div class="absolute left-6 top-1/2 -translate-y-1/2 flex items-center justify-center transition-transform duration-300" :class="isSearchFocused ? 'scale-110' : ''">
                                <i class="bi bi-search text-xl text-gray-500 group-hover:text-rose-400 transition-colors" :class="isSearchFocused ? 'text-rose-400' : ''"></i>
                            </div>
                    ...
                    <div class="flex items-center justify-between gap-4 flex-wrap min-h-[46px]">
                        ...
                        <a href="{{ route('dashboard') }}"
                            class="px-5 py-2 rounded-xl text-sm font-semibold transition-all {{ !request('type') && request('media') !== 'tv' ? 'bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                            {{ __('All') }}
                        </a>
                        @if(($seriesTotal ?? 0) > 0)
                        <a href="{{ route('dashboard', ['media' => 'tv']) }}"
                            class="px-5 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all {{ request('media') === 'tv' ? 'bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                            <i class="bi bi-tv mr-1"></i>{{ __('Series') }}
                        </a>
                        @endif
                        @foreach($collectionTypes as $type)
                            <a href="{{ route('dashboard', ['type' => $type, 'q' => request('q')]) }}"
                                class="px-5 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all {{ request('type') === $type ? 'bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                                {{ $type }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="flex gap-4 items-center shrink-0">
                    ...
                    <div class="flex gap-2 bg-white/5 border border-white/10 p-1.5 rounded-2xl">
                        <button
                            ...
                            :class="viewMode === 'grid' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'text-gray-500 hover:text-white hover:bg-white/5'"
                        >
                        ...
                        <button
                            ...
                            class="p-2 rounded-xl transition-all"
                            :class="viewMode === 'list' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'text-gray-500 hover:text-white hover:bg-white/5'"
                        >
            ...
            <form action="{{ route('dashboard') }}" method="GET"
                  class="px-2 mt-6 glass border {{ $hasActiveFilters ? 'border-rose-500/30' : 'border-white/10' }} rounded-2xl p-5 w-full"
                ...
                <div class="flex items-center gap-2 text-gray-500 self-center shrink-0 pr-2 border-r border-white/10">
                    <i class="bi bi-funnel-fill text-base {{ $hasActiveFilters ? 'text-rose-400' : '' }}"></i>
                ...
                <div style="display:flex;flex-direction:column;gap:0.375rem;">
                    ...
                    <select name="genre" class="bg-white/5 border {{ request('genre') ? 'border-rose-500/50 text-white' : 'border-white/10 text-gray-400' }} rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-rose-500/50 appearance-none cursor-pointer">
                ...
                <div class="flex gap-2 self-end shrink-0">
                    <button type="submit" class="px-5 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all">
            ...
            {{-- Weiterschauen: angefangene Serien mit nächster ungesehener Episode --}}
            ...
            <div class="mt-8 mb-2 px-2">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-black text-white flex items-center gap-4">
                        <div class="h-10 w-10 accent-fill rounded-xl flex items-center justify-center shadow-lg">
                ...
                <div class="flex gap-4 overflow-x-auto no-scrollbar pb-3">
                    @foreach($continueWatching as $serie)
                        <button @click="fetchDetails({{ $serie->id }}, '{{ $serie->backdrop_url }}')"
                                class="w-[240px] shrink-0 group text-left cursor-pointer glass border border-white/10 hover:border-white/30 rounded-2xl overflow-hidden transition-all duration-300">
                            <div class="flex gap-3 p-3">
                                <div class="w-16 aspect-[2/3] rounded-lg overflow-hidden bg-white/5 shrink-0">
                                    @if($serie->cover_url)
                                        <img src="{{ $serie->cover_url }}" alt="{{ $serie->title }}" loading="lazy" class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center text-gray-700"><i class="bi bi-tv text-xl"></i></div>
                                    @endif
                                </div>
                                <div class="min-w-0 flex-1 flex flex-col justify-center gap-1.5">
                                    <p class="text-xs font-black text-white truncate group-hover:accent-text transition-colors">{{ $serie->title }}</p>
                                    ...
                                    <div class="h-1.5 bg-white/10 rounded-full overflow-hidden">
                                        <div class="h-full accent-fill rounded-full" style="width: {{ $serie->progress_percent }}%"></div>
                                    </div>
                                    <p class="text-[9px] font-bold text-gray-500">{{ $serie->progress_watched }} / {{ $serie->progress_total }} {{ __('Episodes') }}</p>
                                </div>
                            </div>
                        </button>
                    @endforeach
            ...
            {{-- Serien-Sektion (getrennt von der Filmliste, nur auf dem ungefilterten Dashboard) --}}
            @if(($series ?? collect())->isNotEmpty())
            <div class="mt-8 mb-2 px-2">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-black text-white flex items-center gap-4">
                        <div class="h-10 w-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg shadow-purple-500/20">
                            <i class="bi bi-tv"></i>
                        </div>
                        <span>{{ __('Series') }}</span>
                        <span class="text-gray-500 text-sm font-bold">{{ $seriesTotal }}</span>
                    </h2>
                    <a href="{{ route('dashboard', ['media' => 'tv']) }}"
                       class="text-xs font-black text-gray-500 hover:text-white uppercase tracking-widest transition-colors">
                        {{ __('Show all') }} <i class="bi bi-arrow-right ml-1"></i>
                    </a>
                </div>
                <div class="flex gap-4 overflow-x-auto no-scrollbar pb-3">
                    @foreach($series as $serie)
                        <button @click="fetchDetails({{ $serie->id }}, '{{ $serie->backdrop_url }}')"
                                class="w-[130px] shrink-0 group text-left cursor-pointer">
                            <div class="aspect-[2/3] rounded-xl overflow-hidden border border-white/10 group-hover:border-purple-500/50 group-hover:scale-[1.03] transition-all duration-300 shadow-lg bg-white/5">
                                @if($serie->cover_url)
                                    <img src="{{ $serie->cover_url }}" alt="{{ $serie->title }}" loading="lazy" class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center text-gray-700"><i class="bi bi-tv text-3xl"></i></div>
                                @endif
                            </div>
                            <p class="mt-2 text-xs font-bold text-gray-300 group-hover:text-white truncate transition-colors">{{ $serie->title }}</p>
                        </button>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex items-center justify-between mb-8 px-2 gap-4 flex-wrap min-h-[40px]">
                <h2 class="text-xl font-black text-white flex items-center gap-4">
                    <div class="h-10 w-10 bg-gradient-to-br from-rose-500 to-rose-600 rounded-xl flex items-center justify-center shadow-lg shadow-rose-500/20">
                        <i class="bi {{ request('media') === 'tv' ? 'bi-tv' : 'bi-collection-play' }}"></i>
                    </div>
                    <span>
                        @if(request('q'))
            ...
            <!-- Film Grid/List -->
            <div x-ref="movieGrid" :class="viewMode === 'grid' ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4' : 'flex flex-col gap-3'">
            ...
            <div x-show="nextMoviesPageUrl"
                ...
                <button x-show="!isMoviesLoading" @click="loadMoreMovies()" class="px-8 py-3 glass border border-white/10 rounded-2xl text-[10px] font-black text-gray-500 hover:text-white hover:border-rose-500/50 transition-all uppercase tracking-[0.2em] italic group">
        ...
        <aside class="detail-panel shadow-2xl block relative min-h-[600px]">
            ...
            <div x-show="error" class="h-full flex flex-col items-center justify-center p-8 text-center">
                ...
                <button @click="error = null" class="mt-4 text-sm text-rose-400 hover:underline">{{ __('Close') }}</button>
            </div>
