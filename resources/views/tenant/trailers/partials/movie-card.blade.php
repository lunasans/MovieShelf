...
<div class="group relative flex flex-col gap-4 animate-in fade-in zoom-in duration-500">
    ...
    <div @click="openTrailer('{{ addslashes($movie->title) }}', '{{ $movie->trailer_url }}')" 
         class="relative aspect-video rounded-[2rem] overflow-hidden {{ $layoutMode === 'streaming' ? 'glass-streaming border-white/10 shadow-2xl skew-x-[-1deg] group-hover:skew-x-0' : 'glass border-white/5' }} transition-all duration-700 hover:scale-[1.05] hover:border-rose-500/50 shadow-2xl group/card cursor-pointer">
    ...
    <!-- Info Section -->
    <div class="px-4">
        <h3 class="text-white font-black text-xl leading-tight tracking-tighter mb-2 truncate group-hover:text-rose-400 transition-colors uppercase italic">
            {{ $movie->title }}
        </h3>
        <div class="flex items-center justify-between">
            <div @click.stop="window.location.href = '{{ route('dashboard', ['movie' => $movie->id]) }}'" 
                 class="flex items-center gap-3 text-[10px] font-black text-white/40 uppercase tracking-[0.2em] cursor-pointer hover:text-rose-400 transition-all italic w-fit group/btn">
                <span class="border-b border-white/10 group-hover/btn:border-rose-500/50 pb-0.5">{{ __('View Details') }}</span>
