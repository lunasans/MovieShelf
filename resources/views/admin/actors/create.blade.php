<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto" x-data="actorForm()">
        ...
        <form action="{{ route('admin.actors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                ...
                <div class="md:col-span-1 space-y-6">
                    <div class="glass p-8 rounded-[3rem] border-white/5 shadow-2xl relative overflow-hidden">
                        <div class="absolute inset-0 bg-gradient-to-t from-rose-600/10 to-transparent pointer-events-none"></div>
                        ...
                        <div class="relative group aspect-square rounded-[2rem] overflow-hidden bg-white/5 border border-white/10 flex flex-col items-center justify-center cursor-pointer hover:border-rose-500/40 transition-all shadow-xl"
                            ...
                            <template x-if="preview">
                                <img :src="preview" class="absolute inset-0 w-full h-full object-cover z-20 transition-all group-hover:scale-105">
                            </template>

                            <div class="relative z-10 flex flex-col items-center gap-4 p-8 text-center">
                                <div class="w-16 h-16 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-400 group-hover:scale-110 transition-transform shadow-lg shadow-rose-500/10 border border-rose-500/20">
                                    <i class="bi bi-person-plus-fill text-2xl"></i>
                                </div>
                                <span class="text-[9px] font-black text-white/40 uppercase tracking-widest leading-tight">Foto hochladen</span>
                            </div>
                        ...
                        <p class="text-[8px] text-white/20 text-center uppercase tracking-widest mt-6">JPG, PNG oder WebP (Max. 4MB)</p>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 space-y-10">
                    <div class="glass p-10 rounded-[3rem] border-white/5 shadow-2xl relative overflow-hidden">
                        ...
                        <h3 class="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-10 flex items-center gap-3">
                            <i class="bi bi-card-list text-rose-500"></i>
                            Informationen
                        </h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="nationality" id="nationality" value="{{ old('nationality') }}" placeholder="z.B. USA"
                            ...
                            <div class="md:col-span-2">
                                <label for="tmdb_id" class="block text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-3 px-1">TMDb Person ID</label>
                                <div class="relative">
                                    <input type="number" name="tmdb_id" id="tmdb_id" value="{{ old('tmdb_id') }}"
                                           placeholder="3223"
                                    ...
                                    <i class="bi bi-database-fill absolute right-6 top-1/2 -translate-y-1/2 text-white/10 text-xl"></i>
                                </div>
                                <p class="text-[9px] text-white/20 mt-3 px-1 italic">Wird automatisch zur Verknüpfung von Filmen genutzt.</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function actorForm() {
            return {
                quill: null,
                formData: {
                    biography: {!! json_encode(old('biography')) !!}
                },
                initQuill() {
                    const setup = () => {
