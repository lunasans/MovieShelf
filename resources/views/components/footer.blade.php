...
@php
    ...
    $showSeriesStat = ($footerStats['total_series'] ?? 0) > 0;
@endphp

<footer class="relative z-10
    {{ $isStreaming ? 'mt-10 pb-20 px-12 md:px-20 bg-gradient-to-t from-[#0c0c0e] to-transparent' : ($compact ? 'py-10 px-12' : 'mt-10 pb-10 px-8') }}">
    ...
    <div class="{{ $isStreaming ? 'max-w-[1400px]' : ($compact ? 'w-full' : 'max-w-7xl') }} mx-auto">
        ...
        <div class="flex flex-col items-center gap-6 mb-8">
            ...
            <!-- Stats Section -->
            <div class="w-full grid grid-cols-2 {{ $showSeriesStat ? 'md:grid-cols-5' : 'md:grid-cols-4' }} gap-6">
                @php
                ...
                <div class="{{ $statBoxClass }} py-6 px-8 rounded-[2rem] flex flex-col items-center justify-center gap-3 transition-all group">
                    ...
                    <div class="text-lg font-black text-white leading-none">{{ number_format($footerStats['total_series']) }}</div>
                    <div class="text-[10px] font-bold {{ $isStreaming ? 'text-white/30' : 'text-gray-500' }} uppercase tracking-widest">{{ __('Series') }}</div>
                </div>
                @endif
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 pt-8">
            <div class="flex items-center gap-8 order-2 md:order-1">
                ...
                <a href="https://github.com/lunasans/MovieShelf" target="_blank" class="text-white/10 hover:text-white transition-all transform hover:scale-125">
