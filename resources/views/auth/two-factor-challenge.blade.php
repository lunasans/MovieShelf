            ...
            <div class="relative">
                ...
                <form method="POST" action="{{ route('two-factor.verify') }}" class="space-y-8">
                    ...
                    <div class="flex flex-col gap-4">
                        <button class="w-full py-4 bg-gradient-to-r from-rose-600 to-red-700 hover:from-rose-500 hover:to-red-600 text-white font-black uppercase tracking-widest rounded-xl shadow-lg shadow-rose-900/40 transition-all transform active:scale-95 justify-center text-base">
                            <span x-show="!useRecovery">{{ __('Verifizieren & Fortfahren') }}</span>
                            <span x-show="useRecovery" x-cloak>{{ __('Backup-Code verwenden') }}</span>
                        </button>
